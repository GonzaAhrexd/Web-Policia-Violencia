        ...
        <Document>
            <Page style={styles.page}>
                ...
                {denunciasAMostrar.map((denuncia: any, index: number) => {
                    ...
                    <View>
                        <Text>{(index +1 )+ ')'} Dcia de fecha {new Date(denuncia.fecha).toLocaleDateString('es-AR')}-{denuncia.jurisdiccion_policial}, Expediente {denuncia.numero_de_expediente} denuncia realizada contra {denuncia.victimario_nombre} </Text>
                    </View>
                </View>) })}
                <View style={styles.section}>
