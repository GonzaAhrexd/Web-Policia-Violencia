...
function InputRegister({ campo, nombre, register, type, error, variante, require, valor, placeholder, setValue }: InputRegisterProps) {
    ...
    return (
        <div className={`flex ${campo=='Barrio' && "flex-col w-full xl:w-1/2"} ${nombre=='numero_de_expediente' && "flex-col w-full"} ${((campo!== 'Barrio' || nombre!='numero_de_expediente' ) && "flex-col md:w-1/2 ")}`}>
